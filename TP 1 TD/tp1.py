...
while not finalizar:
    ...
    if opcion_seleccionada == 'A':
        ...
        resultado:bool = es_cafetero(int(n))
        if resultado:
        ...
        n:str = input('Ingrese n: ')
        resultado:int = n_esimo_cafetero(int(n))
        print('El ' + n +'-ésimo cafetero es el ' + str(resultado) + '.')
    elif opcion_seleccionada == 'D':
        n:str = input ('Ingrese n: ')
        m:str = input ('Ingrese m: ')
        cafeteros:int = cafeteros_entre(int(n),int(m))
        print('Los números cafeteros entre '+ str(n) + ' y ' + str(m) + ' son :'+str(cafeteros)+'.')
